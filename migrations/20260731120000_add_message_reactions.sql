...
-- One emoji reaction per (message, reactor). A member may hold at most one
-- reaction on a message: setting a new emoji replaces the previous one and
-- clearing it removes the row (enforced by the service via an upsert/delete),
-- so the UNIQUE constraint is the source of truth for "one reaction per user".
create table if not exists "im_message"."message_reactions" (
    "id"                uuid        primary key,
    "message_id"        uuid        not null references "im_message"."messages" ("id") on delete cascade,
    "thread_id"         uuid        not null,
    "domain_id"         int         not null,
    "reactor_id"        uuid        not null,
    "reactor_member_id" uuid,
    "emoji"             text        not null,
    "created_at"        timestamptz not null default now(),
    "updated_at"        timestamptz not null default now(),
    constraint "uq_message_reactions_message_reactor" unique ("message_id", "reactor_id")
);

create index if not exists idx_message_reactions_message
    on im_message.message_reactions (message_id);

-- Idempotency-key ledger for reaction writes. A consumed send_id is recorded
-- here, not on the reaction row, so it survives a toggle-off that deletes the
-- row: any at-least-once redelivery of the same request (set, replace or
-- remove) is recognised and becomes a no-op, and a reordered older retry can no
-- longer revert a newer state. Bounded state — purge old rows periodically.
...
-- DEFAULT TRUE keeps existing members able to react to messages: the right is
-- on by default and revoked per member, like can_delete_messages.
alter table "im_thread"."thread_permission"
    add column if not exists "can_react_messages" boolean not null default true;

drop view if exists "im_thread"."v_messages";

-- Rebuilds v_messages (based on 20260727120000) adding the reactions aggregate:
--   reactions - one row per reactor, ordered by first-reaction time, so history
--               renders the current reaction set for both parties.
...
drop index if exists im_message.idx_message_reaction_dedup_created_at;
drop table if exists "im_message"."message_reaction_dedup";

drop index if exists im_message.idx_message_reactions_message;

alter table "im_thread"."thread_permission"
    drop column if exists "can_react_messages";

drop table if exists "im_message"."message_reactions";
