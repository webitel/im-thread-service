...
-- Repurpose the per-message status table into an ERRORS-ONLY table. The only
-- state a monotonic watermark cannot express is FAILED (message X failed while
-- X+1 succeeded); everything else (SENT/DELIVERED/READ) is now a thread_dialog
-- watermark. So message_statuses becomes message_errors, holding only failures.
create table if not exists "im_message"."message_errors" (
    ...
    constraint "pk_message_errors" primary key ("message_id", "member_id"),
    constraint "fk_message_errors_message" foreign key ("message_id")
        references "im_message"."messages" ("id") on delete cascade
);

create index if not exists "idx_message_errors_thread"
    on "im_message"."message_errors" ("thread_id", "member_id", "failed_at");

-- Carry existing failures over (status 4).
insert into "im_message"."message_errors" (domain_id, thread_id, message_id, member_id, failed_at, error, via, updated_at)
select domain_id, thread_id, message_id, member_id, coalesce(failed_at, updated_at), error, via, updated_at
from "im_message"."message_statuses" where status = 4
...
create view "im_thread"."v_messages" as
 SELECT m.id,
    ...
    ( SELECT CASE
             ...
             END
        FROM ( SELECT CASE
                        WHEN EXISTS (SELECT 1 FROM im_message.message_errors e
    ...
    ( SELECT jsonb_agg(jsonb_build_object('member_id', td.member_id, 'status',
                CASE
                    ...
                    WHEN td.last_delivered_message_id IS NOT NULL AND td.last_delivered_message_id >= m.id THEN 2
                    ELSE 1
                END, 'error', e.error) ORDER BY td.member_id)
        FROM im_thread.thread_dialog td
        LEFT JOIN im_message.message_errors e
               ON e.message_id = m.id AND e.member_id = td.member_id
...
create table if not exists "im_message"."message_statuses" (
    ...
    "via"          text,
    "updated_at"   timestamptz not null default now(),
    constraint "pk_message_statuses" primary key ("message_id", "member_id")
);

insert into "im_message"."message_statuses" (domain_id, thread_id, message_id, member_id, status, failed_at, error, via, updated_at)
select domain_id, thread_id, message_id, member_id, 4, failed_at, error, via, updated_at
from "im_message"."message_errors"
on conflict (message_id, member_id) do nothing;
...
drop table if exists "im_message"."message_errors";
