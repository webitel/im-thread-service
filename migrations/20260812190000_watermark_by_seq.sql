...
-- Switch the read/delivered watermarks from message_id (UUIDv7) to the
-- per-thread seq (bigint). seq is a small ordered integer the frontend can
-- reason about, so acks become "delivered/read up to seq N". The message_id
-- horizon columns are kept for one release as a safety net.
alter table "im_thread"."thread_dialog" add column if not exists "last_read_seq" bigint;
alter table "im_thread"."thread_dialog" add column if not exists "last_delivered_seq" bigint;

-- Backfill seq horizons from the existing message_id horizons.
update "im_thread"."thread_dialog" td set
    last_read_seq = (select m.seq from "im_message"."messages" m where m.id = td.last_read_message_id),
    last_delivered_seq = (select m.seq from "im_message"."messages" m where m.id = td.last_delivered_message_id);
...
alter table "im_thread"."thread_dialog" drop column if exists "last_read_seq";
alter table "im_thread"."thread_dialog" drop column if exists "last_delivered_seq";
