...
-- Catch-up for GetUpdates. Idempotent: dev already has parts of an earlier draft of this.

-- Per-thread counter of journaled mutations (distinct from last_seq, which orders messages).
alter table "im_thread"."thread"
    add column if not exists "last_update_seq" bigint not null default 0;

-- One row per mutation, written in the mutation's transaction; tx_id scopes it to cursors.
create table if not exists "im_message"."thread_updates" (
...
-- Added without a default first so pre-existing draft rows stay null (then deleted below).
alter table "im_message"."thread_updates" add column if not exists "tx_id" bigint;
alter table "im_message"."thread_updates"
    alter column "tx_id" set default (pg_current_xact_id()::text::bigint);
-- Insert time keeps created_at monotonic with update_seq, so retention trims a prefix.
alter table "im_message"."thread_updates"
...
create index if not exists "idx_thread_updates_created_at"
    on "im_message"."thread_updates" ("created_at");

-- Leftovers of the draft: body redaction index and rows GetUpdates can never serve.
drop index if exists "im_message"."idx_thread_updates_thread_msg";
delete from "im_message"."thread_updates" where "tx_id" is null;

-- Newest transaction whose rows retention deleted; older cursors resync.
create table if not exists "im_message"."thread_updates_trim" (
    "id"    smallint primary key default 1 check ("id" = 1),
    "tx_id" bigint   not null
);

-- Which threads changed for a contact, and in which transaction; one row per (contact, thread).
create table if not exists "im_thread"."contact_updates" (
    "contact_id" uuid   not null,
    "thread_id"  uuid   not null,
    "tx_id"      bigint not null,
    constraint "pk_contact_updates" primary key ("contact_id", "thread_id")
);

create index if not exists "idx_contact_updates_contact_tx"
    on "im_thread"."contact_updates" ("contact_id", "tx_id");

-- History no longer returns delivery_status/statuses; drop their per-row subqueries.
...
create view "im_thread"."v_messages" as
 SELECT m.id,
    ...
    ( SELECT jsonb_build_object('message_id', r.id, 'sender_id', r.sender_id, 'type', r.type, 'body',
                ...
                CASE
                    ...
                    ELSE COALESCE(( SELECT jsonb_build_object('kind', 'document', 'name', d.name, 'mime', d.mime) AS jsonb_build_object
                     ...
                     LIMIT 1), ( SELECT jsonb_build_object('kind', 'location', 'name', NULLIF(l.name, ''::text), 'address', NULLIF(l.address, ''::text)) AS jsonb_build_object
    ...
    ( SELECT jsonb_build_object('message_id', r.id, 'sender_id', r.sender_id, 'type', r.type, 'body', "left"(COALESCE(r.body, ''::text), 256), 'created_at', (EXTRACT(epoch FROM r.created_at) * 1000::numeric)::bigint, 'is_deleted', r.deleted_at IS NOT NULL, 'attachment', COALESCE(( SELECT jsonb_build_object('kind', 'document', 'name', d.name, 'mime', d.mime) AS jsonb_build_object
                       FROM im_message.message_documents d
                      WHERE d.message_id = r.id
                      ORDER BY d.created_at
                     LIMIT 1), ( SELECT jsonb_build_object('kind', 'image', 'mime', i.mime) AS jsonb_build_object
           ...
           FROM im_message.messages r
          WHERE r.id = m.reply_to) AS reply_to_audit,
        CASE
...
drop table if exists "im_thread"."contact_updates";
drop table if exists "im_message"."thread_updates_trim";
drop table if exists "im_message"."thread_updates";
