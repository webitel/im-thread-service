...
SELECT m.id,
    ...
    CASE WHEN m.deleted_at IS NOT NULL THEN NULL ELSE m.metadata END AS metadata,
    m.created_at,
    m.updated_at,
    m.edited,
    m.deleted_at,
    m.deleted_by,
	CASE WHEN m.deleted_at IS NOT NULL THEN NULL ELSE to_jsonb(bc.*) END as reacted_metadata,
    ...
    ( SELECT jsonb_agg(unnamed_subquery.document) AS jsonb_agg
           FROM ( SELECT jsonb_build_object('id', doc.id, 'message_id', doc.message_id, 'file_id', doc.file_id, 'name', doc.name, 'mime', doc.mime, 'size', doc.size, 'created_at', doc.created_at) AS document
                  ...
                  WHERE (m.type = 2 OR m.type = 5 AND ((m.interactive -> 'attachments'::text) -> 'documents'::text) IS NOT NULL) AND doc.message_id = m.id) unnamed_subquery) END AS documents,
    CASE WHEN m.deleted_at IS NOT NULL THEN NULL ELSE
    ( SELECT jsonb_agg(unnamed_subquery.image) AS jsonb_agg
           FROM ( SELECT jsonb_build_object('id', img.id, 'message_id', img.message_id, 'file_id', img.file_id, 'mime', img.mime, 'width', img.width, 'height', img.height, 'created_at', img.created_at) AS image
                  ...
                  WHERE (m.type = 3 OR m.type = 5 AND ((m.interactive -> 'attachments'::text) -> 'images'::text) IS NOT NULL) AND img.message_id = m.id) unnamed_subquery) END AS images,
    CASE WHEN m.deleted_at IS NOT NULL THEN NULL ELSE
    ...
    ( SELECT jsonb_build_object('address', ml.address, 'name', ml.name, 'latitude', ml.latitude, 'longitude', ml.longitude) AS jsonb_build_object
         ...
         LIMIT 1) END AS location,
    m.domain_id,
    tm.member_data AS member,
    CASE WHEN m.deleted_at IS NOT NULL THEN NULL ELSE m.interactive END AS interactive,
    CASE WHEN m.deleted_at IS NOT NULL THEN NULL ELSE
    ...
    ( SELECT jsonb_build_object(
      ...
      WHERE r.id = m.reply_to) END AS reply_to,
    CASE WHEN m.deleted_at IS NOT NULL OR m.forward_origin_kind IS NULL THEN NULL ELSE
        jsonb_build_object(
    ...
    ( SELECT jsonb_agg(jsonb_build_object(
           ...
           FROM ( SELECT mr.emoji,
                   ...
                   GROUP BY mr.emoji ) e ) END AS reactions
