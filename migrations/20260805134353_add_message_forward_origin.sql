...
alter table "im_message"."messages"
    ...
    add column if not exists "forward_origin_sender_name" text,
    add column if not exists "forward_origin_sent_at"     timestamptz,
...
create index if not exists idx_messages_forward_from
    on im_message.messages (forward_from_message_id)
...
drop index if exists im_message.idx_messages_forward_from;

alter table "im_message"."messages"
    drop column if exists "forward_origin_kind",
    drop column if exists "forward_origin_sender_id",
    drop column if exists "forward_origin_sender_name",
    drop column if exists "forward_origin_sent_at",
