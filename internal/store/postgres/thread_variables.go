...
func prepareThreadVariablesSetQuery(variables *model.SetThreadVariablesCommand) (string, pgx.NamedArgs) {
	query := `
		...
		where exists (
			...
			where (td.thread_id, td.member_id) = (@ThreadID, @Member)
		)
...
func prepareThreadVariablesFLushQuery(flushCmd model.FlushVariablesCommand) (string, pgx.NamedArgs) {
	query := `
		with validation as (
			select thread_id from im_thread.thread_variables
		...
		set variables = (
			...
			from jsonb_each(tv.variables) as t(key, val)
			where not (
		...
		from validation v
		where tv.thread_id = v.thread_id
		and exists (
			select 1
			from im_thread.thread_dialog td
			where (td.thread_id, td.member_id) = (v.thread_id, @Member)
		)
		returning tv.thread_id, tv.variables;
	`
